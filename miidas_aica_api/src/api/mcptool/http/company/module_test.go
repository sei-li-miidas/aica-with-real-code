...
type stubGetDetailUseCase struct {
	execute func(positionID position.ID) (*companyUC.GetDetailResponse, error)
}

func (s *stubGetDetailUseCase) Execute(positionID position.ID) (*companyUC.GetDetailResponse, error) {
	...
	t.Run("success", func(t *testing.T) {
		module, err := NewModule(Dependencies{
			NewGetDetailUseCase: func(_ logger.LevelLogger) GetDetailUseCase {
				return &stubGetDetailUseCase{execute: func(position.ID) (*companyUC.GetDetailResponse, error) {
					return &companyUC.GetDetailResponse{}, nil
				}}
			},
		})
...
func TestSetup(t *testing.T) {
	t.Run("error when module is nil", func(t *testing.T) {
		e := echo.New()
		err := Setup(e, nil)
		assert.EqualError(t, err, "company module handler is nil")
	})
	...
	t.Run("register route", func(t *testing.T) {
		...
		for _, r := range e.Routes() {
			if r.Method == http.MethodGet && r.Path == "/companies/detail/position_id/:position_id" {
				found = true
				break
			}
		}
...
func TestHandlerGetDetail(t *testing.T) {
	t.Run("invalid position_id", func(t *testing.T) {
		e := echo.New()
		req := httptest.NewRequest(http.MethodGet, "/companies/detail/position_id/abc", nil)
	...
	t.Run("usecase error", func(t *testing.T) {
		e := echo.New()
		req := httptest.NewRequest(http.MethodGet, "/companies/detail/position_id/1", nil)
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		c.SetParamNames("position_id")
		c.SetParamValues("1")
		...
		h := NewHandler(func(_ logger.LevelLogger) GetDetailUseCase {
			return &stubGetDetailUseCase{
				execute: func(_ position.ID) (*companyUC.GetDetailResponse, error) {
	...
	t.Run("success", func(t *testing.T) {
		...
		assert.True(t, strings.Contains(rec.Body.String(), "ID"))
	})
}
