# タスク: task-2-responses-runner-adapter
...
- `LLMRunStream` / `ResponsesRunStream` / `ResponsesAgentRunner` を追加し、SDK-shaped event を normalized contract に変換する。
- `Runner.run_streamed(..., previous_response_id=...)` の forwarding と `tool_replay_items` / `usage` の contract を test で固定する。
